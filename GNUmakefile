...
# YES I AM USING WSL cry me a river
.PHONY: run-win
...
.PHONY: run
run: $(IMAGE_NAME).iso
	qemu-system-x86_64 $(QEMU_FALGS) -cdrom $(IMAGE_NAME).iso -boot d

.PHONY: run-uefi
run-uefi: ovmf $(IMAGE_NAME).iso
	qemu-system-x86_64 $(QEMU_FALGS) -bios ovmf/OVMF.fd -cdrom $(IMAGE_NAME).iso -boot d

.PHONY: run-hdd
run-hdd: $(IMAGE_NAME).hdd
	qemu-system-x86_64 $(QEMU_FALGS) -hda $(IMAGE_NAME).hdd

.PHONY: run-hdd-uefi
run-hdd-uefi: ovmf $(IMAGE_NAME).hdd
	qemu-system-x86_64 $(QEMU_FALGS) -bios ovmf/OVMF.fd -hda $(IMAGE_NAME).hdd
